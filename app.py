...
st.markdown("""
    ...
    <style>
        ...
        .stButton>button { border-radius: 10px; border: 1px solid #30363D; background: #21262D; color: white; transition: 0.2s; }
...
st.markdown(f"""
    <div class="main-card">
        ...
        <div style="display: flex; justify-content: space-around; margin-top: 15px; border-top: 1px solid #30363D; padding-top: 10px;">
            <div><div class="stat-label">TOPLAM GELİR</div><div style="color:#3FB950; font-weight:bold;">{total_gelir:,.0f}</div></div>
            <div><div class="stat-label">TOPLAM GİDER</div><div style="color:#F85149; font-weight:bold;">{total_gider:,.0f}</div></div>
        </div>
    </div>
""", unsafe_allow_html=True)
...
if st.session_state.page == "Özet":
    ...
    st.subheader(f"📍 {curr_month} Ayı Aidat Takibi")
    ...
    c1, c2 = st.columns(2)
    c1.metric("Ödeyen", f"{len(yapanlar)}")
    c2.metric("Kalan", f"{len(borclular)}", delta=-len(borclular), delta_color="inverse")
    
    if borclular:
        st.error(f"Henüz Ödemeyenler: {', '.join(borclular)}")
    else: st.success("Bu ay tüm aidatlar ödendi!")

# --- SAYFA: İŞLEM (Hata Düzeltildi) ---
elif st.session_state.page == "İşlem":
    if not is_admin:
        st.warning("İzleyici yetkisiyle işlem yapılamaz.")
    else:
        secilen_ay = st.selectbox("İşlem Ayı", aylar, index=datetime.now().month-1)
        # HATA DÜZELTİLDİ: "default" değeri listedeki ile birebir aynı yapıldı
        islem_tipi = st.selectbox("İşlem Türü", ["Gelir Tahsilatı", "Gider Ödemesi"])
        ...
        if islem_tipi == "Gelir Tahsilatı":
            with st.form("gelir_form", clear_on_submit=True):
                ...
                t_tur = st.selectbox("Tahsilat Kalemi", ["Aidat", "Asansör Bakımı", "Ek Ödeme"])
                t_miktar = st.number_input("Daire Başı Tutar", value=400)
                t_tarih = st.date_input("İşlem Tarihi")
                if st.form_submit_button("SİSTEME KAYDET", use_container_width=True):
                    if sec_daireler:
                        yeni_data = [{"Tarih": t_tarih.strftime("%d.%m.%Y"), "Ay": secilen_ay, "Daire": d, "Tür": t_tur, "Miktar": t_miktar} for d in sec_daireler]
                    ...
                    else: st.warning("Daire seçimi yapmadınız!")

        else:
            with st.form("gider_form", clear_on_submit=True):
                g_aciklama = st.text_input("Gider Açıklaması")
                ...
                if st.form_submit_button("GİDERİ KAYDET", use_container_width=True):
                    if g_aciklama:
                        ...
                        if veri_kaydet("Giderler", pd.concat([df_gider, yeni_gider], ignore_index=True)):
                            st.success("Gider İşlendi!"); time.sleep(1); st.rerun()
                    else: st.warning("Lütfen açıklama girin!")

# --- SAYFA: RAPOR & SİLME ---
elif st.session_state.page == "Rapor":
    r_ay = st.selectbox("Ay Filtresi", ["Tümü"] + aylar)
