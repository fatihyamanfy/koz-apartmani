...
def verileri_yukle():
    try:
        ...
        # SÜTUN İSİMLERİNİ STANDARTLAŞTIR (Hata Almamak İçin)
        # Tablodaki 'miktar', 'TUTAR', 'ay' gibi farklı yazımları düzeltir
        ...
        # Tablo boşsa veya ulaşılamazsa boş taslak oluştur
...
for idx, ay_adi in enumerate(aylar):
    # 'Ay' ve 'Miktar' sütunları varsa hesapla
...
# Bakiye
t_gelir = df_gelir["Miktar"].sum() if "Miktar" in df_gelir.columns else 0
t_gider = df_gider["Miktar"].sum() if "Miktar" in df_gider.columns else 0
...
with t_aylik:
    ...
    with c1:
        ...
        if "Ay" in df_gelir.columns:
            st.dataframe(df_gelir[df_gelir["Ay"] == secilen_ay], use_container_width=True, hide_index=False)
    ...
    with c2:
        ...
        if "Ay" in df_gider.columns:
            st.dataframe(df_gider[df_gider["Ay"] == secilen_ay], use_container_width=True, hide_index=False)
