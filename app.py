...
if st.session_state.logged_in_user is None:
    ...
    with t_up:
        st.info("Kayıt sadece izleme yetkisi verir.")
        st.text_input("Yeni Kullanıcı")
        st.text_input("6 Haneli Şifre ", max_chars=6)
...
with t_yon:
    ...
    if is_admin:
        with st.expander("➕ Yeni Gelir Kaydet"):
            ...
            if st.button("KAYDET"):
                t_fix = f"20.{ay_no_map[secilen_ay]}.2026"
                yeni = [{"Tarih": t_fix, "Ay": secilen_ay, "Daire": d, "Tür": g_tur, "Miktar": g_tutar} for d in daireler]
    ...
    for idx, row in f_gelir.iterrows():
        ...
        if is_admin and c1.button("🗑️", key=f"g_del_{idx}"):
    ...
    if is_admin:
        with st.expander("➕ Yeni Gider Kaydet"):
            ...
            if st.button("GİDERİ KAYDET"):
                t_fix = f"20.{ay_no_map[secilen_ay]}.2026"
                yeni_g = pd.DataFrame([{"Tarih": t_fix, "Ay": secilen_ay, "Tür": acik, "Miktar": tut}])
    ...
    for idx, row in f_gider.iterrows():
        c1, c2 = st.columns([0.15, 0.85])
        if is_admin and c1.button("🗑️", key=f"gid_del_{idx}"):
...
with t_borc:
    ...
    borclar = [d for d in DAIRE_LISTESI if d not in yapanlar]
    for b in borclar: st.warning(f"❌ {b}")
    if not borclar: st.success("Herkes ödedi!")
